...
export const ChannelSearchStore = {
  ...
  /** Ghi/cập nhật dữ liệu NÔNG cho mọi kênh vừa đi qua lượt tìm. COALESCE để giá trị
   *  mới null không đè mất dữ liệu cũ đã có (vd lần trước có subs, lần này nguồn
   *  yt-dlp không trả). */
  poolUpsertShallow(rows: CsSearchResult[]): void {
    if (!rows.length) return
    const now = Date.now()
    const st = getDb().prepare(`
      INSERT INTO cs_pool (yt_channel_id, url, name, handle, thumbnail, subs, video_count,
        country, yt_created_at, topics, shorts_count, first_seen_at, updated_at)
      VALUES (@id, @url, @name, @handle, @thumbnail, @subs, @videoCount, @country, @createdAt,
        @topics, @shortsCount, @now, @now)
      ON CONFLICT(yt_channel_id) DO UPDATE SET
        ...
        subs = COALESCE(excluded.subs, subs),
        video_count = COALESCE(excluded.video_count, video_count),
        country = COALESCE(excluded.country, country),
        yt_created_at = COALESCE(excluded.yt_created_at, yt_created_at),
        topics = COALESCE(excluded.topics, topics),
        shorts_count = COALESCE(excluded.shorts_count, shorts_count),
        updated_at = excluded.updated_at
    `)
    const tx = getDb().transaction((list: CsSearchResult[]) => {
      for (const r of list) {
        st.run({
          id: r.ytChannelId, url: r.url, name: r.name, handle: r.handle, thumbnail: r.thumbnail,
          subs: r.subs, videoCount: r.videoCount, country: r.country, createdAt: r.ytCreatedAt,
          topics: r.topics ? JSON.stringify(r.topics) : null, shortsCount: r.shortsCount, now
        })
      }
    })
    tx(rows)
  ...
  /** Ghi chỉ số SÂU sau fetchDeep. Gọi TRƯỚC khi xóa __medianDur khỏi kết quả. */
  poolUpsertDeep(rows: CsSearchResult[]): void {
    if (!rows.length) return
    this.poolUpsertShallow(rows) // bảo đảm dòng tồn tại + phần nông mới nhất
    const now = Date.now()
    const st = getDb().prepare(`
      UPDATE cs_pool SET
        avg_views = COALESCE(@avgViews, avg_views),
        last_upload_at = COALESCE(@lastUploadAt, last_upload_at),
        uploads_per_week = COALESCE(@uploadsPerWeek, uploads_per_week),
        like_view_pct = COALESCE(@likeViewPct, like_view_pct),
        comment_view_pct = COALESCE(@commentViewPct, comment_view_pct),
        view_sub_ratio = COALESCE(@viewSubRatio, view_sub_ratio),
        momentum_pct = COALESCE(@momentumPct, momentum_pct),
        view_consistency = COALESCE(@viewConsistency, view_consistency),
        audience_langs = COALESCE(@audienceLangs, audience_langs),
        sample_videos = COALESCE(@sampleVideos, sample_videos),
        median_dur = COALESCE(@medianDur, median_dur),
        deep_at = @now, updated_at = @now
      WHERE yt_channel_id = @id
    `)
    const tx = getDb().transaction((list: CsSearchResult[]) => {
      for (const r of list) {
        st.run({
          id: r.ytChannelId, avgViews: r.avgViews, lastUploadAt: r.lastUploadAt,
          uploadsPerWeek: r.uploadsPerWeek, likeViewPct: r.likeViewPct,
          commentViewPct: r.commentViewPct, viewSubRatio: r.viewSubRatio,
          momentumPct: r.momentumPct, viewConsistency: r.viewConsistency,
          audienceLangs: r.audienceLangs ? JSON.stringify(r.audienceLangs) : null,
          sampleVideos: r.sampleVideos ? JSON.stringify(r.sampleVideos) : null,
          medianDur: (r as unknown as { __medianDur?: number }).__medianDur ?? null,
          now
        })
      }
    })
  ...
  poolCount(): number {
    return (getDb().prepare('SELECT COUNT(*) c FROM cs_pool').get() as { c: number }).c
  },

  /** Toàn bộ kho, dạng CsSearchResult (+__medianDur để lọc thời lượng). */
  poolAll(): CsSearchResult[] {
    const rows = getDb().prepare('SELECT * FROM cs_pool ORDER BY updated_at DESC').all() as any[]
    return rows.map((r) => {
      const c: CsSearchResult = {
        ytChannelId: r.yt_channel_id, url: r.url, name: r.name, handle: r.handle,
        thumbnail: r.thumbnail, subs: r.subs, videoCount: r.video_count,
        country: r.country, ytCreatedAt: r.yt_created_at,
        topics: r.topics ? JSON.parse(r.topics) : null,
        avgViews: r.avg_views, lastUploadAt: r.last_upload_at, uploadsPerWeek: r.uploads_per_week,
        likeViewPct: r.like_view_pct, commentViewPct: r.comment_view_pct,
        viewSubRatio: r.view_sub_ratio, momentumPct: r.momentum_pct,
        viewConsistency: r.view_consistency, shortsCount: r.shorts_count,
        audienceLangs: r.audience_langs ? JSON.parse(r.audience_langs) : null,
        sampleVideos: r.sample_videos ? JSON.parse(r.sample_videos) : null
      }
      if (r.median_dur !== null) (c as unknown as { __medianDur?: number }).__medianDur = r.median_dur
      return c
    })
  },
