...
export function NewProfileDialog({
  ...
  const submit = async (): Promise<void> => {
    ...
    setSaving(true)
    try {
      await window.hnv.profiles.create({
  ...
  const importTxt = async (): Promise<void> => {
    ...
    try {
      ...
      showToast(`Đã tạo ${res.created} hồ sơ${res.failed > 0 ? `, ${res.failed} dòng lỗi/bỏ qua` : ''}`, 'success')
      onCreated()
    } catch (e: any) {
    ...
    <Overlay onClose={onClose}>
      <div className="w-[560px] bg-[#0d0e14] border border-border rounded-[14px] shadow-2xl overflow-hidden">
        <div className="px-[22px] py-[18px] border-b border-borderSoft flex items-center">
          <div className="text-[18px] font-bold">Tạo Hồ sơ mới</div>
        ...
        <div className="px-[22px] pt-[18px] flex gap-2.5">
          <button
            ...
            className={
              'flex-1 rounded-[10px] px-3.5 py-2.5 text-[14px] border ' +
              (mode === 'manual'
          ...
          >
            <Icon name="edit" filled size={15} className="inline align-[-3px] mr-1" />
            Thủ công
          </button>
          <button
            onClick={() => setMode('txt')}
          ...
          >
            <Icon name="folder" filled size={15} className="inline align-[-3px] mr-1" />
            Từ file .txt
          </button>
        </div>
        ...
        {mode === 'manual' ? (
          <>
            <div className="hv-scroll px-[22px] py-[22px] max-h-[420px] overflow-y-auto">
              <Field label="Tên hồ sơ (tiền tố khi tạo nhiều)">
                <input className="inp" value={name} onChange={(e) => setName(e.target.value)} />
                {qty > 1 && (
                  <div className="text-[12px] text-muted mt-1.5">
                    Sẽ tạo: {name}1, {name}2, … {name}
                    {qty}
                  </div>
                )}
              </Field>
              ...
              <div className="bg-card border border-borderSoft rounded-[11px] p-4 mb-4">
                <div className="flex items-center mb-3">
                  <div className="text-[14px] font-semibold">Số lượng hồ sơ</div>
              ...
              <Field label="Proxy">
                ...
                <div className="text-[12px] text-muted mt-1.5">
                  <Icon name="info" filled size={14} className="inline align-[-3px] mr-1" />
                  Gán proxy riêng cho từng hồ sơ sau, trong Cài đặt.
                </div>
              </Field>

              <div className="bg-card border border-borderSoft rounded-[11px] p-3.5 mb-4">
                <div className="text-[14px] font-semibold">
                  <Icon name="fingerprint" filled size={15} className="inline align-[-3px] mr-1" />
                  Fingerprint <span className="text-[12px] text-ok font-normal ml-1.5">tự sinh ngẫu nhiên</span>
                </div>
                <div className="text-[12px] text-muted mt-2">
                  OS / trình duyệt / màn hình / WebGL / canvas… random độc lập cho từng hồ sơ.
                </div>
              </div>

              <Field label="Ghi chú">
                <textarea
                  className="inp h-[54px] resize-none"
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="vd: email, ghi chú riêng..."
                />
              </Field>
            </div>
            ...
            <div className="px-[22px] py-4 border-t border-borderSoft flex gap-2.5 justify-end items-center">
              <div className="mr-auto text-[13px] text-muted">
                Sẽ tạo <b className="text-accent2">{qty}</b> hồ sơ
              </div>
              <button
                onClick={onClose}
            ...
            <div className="px-[22px] py-[22px]">
              <div className="bg-card border border-borderSoft rounded-[11px] p-4 mb-4">
                ...
                <div className="text-[12px] text-muted space-y-1">
                  <div>• Mỗi dòng = 1 account = 1 hồ sơ</div>
                  <div>• Tên hồ sơ = username</div>
